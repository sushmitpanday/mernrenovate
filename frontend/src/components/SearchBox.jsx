...
const SearchBox = ({ onSelect }) => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const res = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(value)}&countrycodes=au&limit=5&addressdetails=1`);
        ...
        <ul className="absolute z-[9999] bg-white border border-gray-200 w-full mt-1 rounded-lg shadow-xl max-h-60 overflow-y-auto">
          {results.map((item, index) => (
            <li 
              ...
              onClick={() => {
                if (onSelect) onSelect(item.display_name);
