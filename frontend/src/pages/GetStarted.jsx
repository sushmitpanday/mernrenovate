    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-lg relative">
        ...
        <div className="w-full bg-gray-200 h-1.5 mb-8 rounded-full overflow-hidden">
          <div className="bg-orange-600 h-full transition-all duration-300" style={{ width: `${progress}%` }}></div>
        </div>
        ...
        {/* Step 2: Service */}
        {step === 2 && (
          <select className="w-full p-4 border rounded-xl mb-6 bg-white" onChange={(e) => setFormData({...formData, service: e.target.value})}>
        ...
        <div className="flex gap-4">
          {step > 1 && (
            <button onClick={handleBack} className="flex-1 py-3 border rounded-xl font-bold">Back</button>
          )}
          <button 
            onClick={handleNext}
